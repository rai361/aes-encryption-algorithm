...
fn euclid_gcf(mut a: u8, mut b: u8) -> u8 {
    while true {
...
fn extended_euclid_gcf(a: u8, b: u8) -> u8 {
...
// potential optimization allowing shift row swaps to be skipped
// but maybe at a memory expense
fn shift_rows_and_mix_columns(block: &mut [u8; 16]) {
    let mut values: [u8; 4];
...
fn print_block_from_hex_string(plaintext: &str) {
...
fn hex_to_be_bytes(plaintext: &str) -> [u8; 16] {
